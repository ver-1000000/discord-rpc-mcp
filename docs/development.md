...
The executable is written to `dist/discord-rpc-mcp` (`.exe` on Windows) for the build host's architecture. CI builds Linux x64 (glibc), Windows x64 and macOS arm64/x64, tests the standalone executables and uploads Actions artifacts. Windows and macOS builds embed the native credential store module. No prebuilt GitHub Release is published yet. Use `node src/cli.js` to run from source.
